...
def venue_serializer(venue):
    ...
    data = {
        ...
        'past_shows': [show_serializer(show, Venue) for show in past_shows_query.all()],
        'upcoming_shows': [show_serializer(show, Venue) for show in upcoming_shows_query.all()],
...
def artist_serializer(artist):
    ...
    data = {
        ...
        'past_shows': [show_serializer(show, Artist) for show in past_shows_query.all()],
        'upcoming_shows': [show_serializer(show, Artist) for show in upcoming_shows_query.all()],
        "past_shows_count": past_shows_query.count(),
        "upcoming_shows_count": upcoming_shows_query.count()
    }
    return data
...
#  Shows
#  ----------------------------------------------------------------
def show_serializer(show, content_type: db.Model = None):
    data = {
    ...
    if isinstance(content_type, Artist):
        data.pop('artist_id')
        data.pop('artist_name')
        data.pop('artist_image')
        data['venue_image'] = show.venue.image_link

    if isinstance(content_type, Venue):
        data.pop('venue_id')
        data.pop('venue_name')
